    ...
    <PanelCard accent="canary" className="space-y-3 pt-5">
      {queued && (
        <div className="space-y-2 rounded-sm border-2 border-ink bg-canary p-2.5 text-xs text-[#383838]">
          <p>{t('ideas.queued', { handle: queued.tweet.authorHandle })}</p>
      ...
      {capture?.status === 'loading' && (
        <p className="t-shimmer text-xs" data-text={t('ideas.loading', { who })}>{t('ideas.loading', { who })}</p>
      )}
      {capture?.status === 'error' && (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={discard}>{t('common.dismiss')}</Button>
          </div>
        </div>
      )}
      {capture?.status === 'duplicate' && (
        <div className="flex items-center gap-2">
          <p className="flex-1">{t('ideas.duplicate')}</p>
          <Button size="sm" onClick={() => { onShowExisting(capture.existingId!); discard(); }}>{t('ideas.showIt')}</Button>
        </div>
      )}
      {capture?.status === 'ready' && capture.draft && (
        <>
          <p className="text-xs text-ink-muted">{t('ideas.ready', { who })}</p>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={() => void save()}>{t('ideas.save')}</Button>
            <Button size="sm" variant="ghost" onClick={discard}>{t('common.discard')}</Button>
          </div>
        </>
      )}
    </PanelCard>
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <SegmentedTabs
          label={t('ideas.filter')}
          options={(['all', ...IDEA_STATUSES] as const).map((f) => ({ value: f, label: `${t(`status.${f}`)} ${count(f)}` }))}
          value={filter}
          onChange={setFilter}
        />
